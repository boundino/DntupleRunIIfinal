...
TF1* fit(TH1D* h, TH1D* hMCSignal, TH1D* hMCSwapped, Double_t ptmin, Double_t ptmax, bool isPbPb, TF1* &total, Float_t centmin, Float_t centmax)
{
  ...
  texcol->Draw();
  TLatex* texpaper;
  texpaper = new TLatex(0.93,0.76,Form("%.0f < p_{T} < %.0f GeV/c",ptmin,ptmax));
  if(ptmin>12 && ptmin<13) texpaper = new TLatex(0.93,0.76,Form("%.1f < p_{T} < %.0f GeV/c",ptmin,ptmax));
  if(ptmax>12 && ptmax<13) texpaper = new TLatex(0.93,0.76,Form("%.0f < p_{T} < %.1f GeV/c",ptmin,ptmax));
  texpaper->SetNDC();
  texpaper->SetTextAlign(32);
  texpaper->SetTextFont(42);
  texpaper->SetTextSize(0.050);
