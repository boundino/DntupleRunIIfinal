...
void CombineRAA(TString fileMB="ROOTfilesCent10/outputRAAMB.root", TString file="ROOTfilesCent10/outputRAA.root", 
                Float_t centmin=0., Float_t centmax=10., 
                Int_t isHadDupl=0, Int_t isB=0, Int_t isNjpsi=0, Int_t isTheoryComparison=0, Int_t isAlice=0, 
                Bool_t verbose=true)
{
  ...
  texlumi->Draw();
  TLatex* texcmspaper = new TLatex(CMSxposRAA, CMSyposRAA, "CMS");
  settex(texcmspaper, CMSsizeRAA, 13, 62);
  TLatex* texcmssupp = new TLatex(CMSxposRAA, CMSyposRAA, "#bf{CMS} #scale[0.6]{#it{Supplementary}}");
  settex(texcmssupp, CMSsizeRAA, 13);
  TLatex* texcms = (isTheoryComparison==0&&isAlice==0&&((isHadDupl==0&&isB==0&&isNjpsi==0)||(isHadDupl==1&&isB==1&&(isNjpsi==0||isNjpsi==2))))?texcmssupp:texcmspaper;
  texcms->Draw();
  ...
  if(!(isB==1||isNjpsi>0||isHadDupl==1)) texDzero->Draw();
  TString texper="%";
 ...
 Double_t legx1[]      = {0.654,          0.594,          0.554,          0.574,          0.574,          0.594,          0.594};
 Double_t legy1[]      = {0.800,          0.760,          0.617,          0.647,          0.597,          0.680,          0.617};
 Double_t legx2[]      = {legx1[0]+0.419, legx1[1]+0.460, legx1[2]+0.460, legx1[3]+0.389, legx1[4]+0.389, legx1[5]+0.460, legx1[6]+0.460};
 Double_t legy2[]      = {0.890,          0.890,          0.906,          0.906,          0.906,          0.890,          0.906};
 Double_t legtexsize[] = {0.043,          0.035,          0.035,          0.030,          0.030,          0.035,          0.035};

  Int_t pindx = -1;
  // D RAA only
  if(isHadDupl==0 && isTheoryComparison==0) pindx = 0;
  // D and charged particles || compare Alice
  if((isHadDupl==1 && isB==0 &&isNjpsi==0) || (isAlice==1 && isHadDupl==0 && isB==0 && isNjpsi==0)) pindx = 1;
  // D, B, jpsi (2.76 TeV) and charged particles
  if(isHadDupl==1 && isB==1 && isNjpsi==1) pindx = 2;
  // separate theories
  if(isTheoryComparison==2||isTheoryComparison==3) pindx = 3;
  // all theories
  if(isTheoryComparison==1) pindx = 4;
  // D, B and charged particles
  if(isHadDupl==1 && isB==1 && isNjpsi==0) pindx = 5;
  // D, B, jpsi (5.02 TeV) and charged particles
  if(isHadDupl==1 && isB==1 && isNjpsi==2) pindx = 6;
  if(pindx<0) return;

  TLegend* legendRAA = new TLegend(legx1[pindx], legy1[pindx], legx2[pindx], legy2[pindx], "");
  setleg(legendRAA, legtexsize[pindx]);
  
  TString str_ent_Dzero = (isHadDupl==1 || isB==1 || isNjpsi>0)?"D#scale[0.6]{#lower[-0.7]{0}} + #bar{D}#scale[0.6]{#lower[-0.7]{0}}":"R_{AA}";
  ...
  if(isNjpsi==1 && centmin==0 && centmax==100)
    {
      legendRAA->AddEntry(gre2TeV2, "1.6 < |y| < 2.4", "pf");
      legendRAA->AddEntry(gre2TeV, "|y| < 2.4", "pf");
      TLatex* texNJpsi2TeV = new TLatex(0.57, 0.728, "nonprompt J/#psi (2.76 TeV)");
      texNJpsi2TeV->SetNDC();
      texNJpsi2TeV->SetTextColor(1);
      texNJpsi2TeV->SetTextFont(42);
      texNJpsi2TeV->SetTextSize(0.035);
      texNJpsi2TeV->Draw();
    }

  if(isNjpsi==2 && centmin==0 && centmax==100)
    {
      legendRAA->AddEntry(gre5TeV2, "1.8 < |y| < 2.4", "pf");
      legendRAA->AddEntry(gre5TeV, "|y| < 2.4", "pf");
      TLatex* texNJpsi5TeV = new TLatex(0.57+0.04, 0.728, "nonprompt J/#psi");
      texNJpsi5TeV->SetNDC();
      texNJpsi5TeV->SetTextColor(1);
      texNJpsi5TeV->SetTextFont(42);
      texNJpsi5TeV->SetTextSize(0.035);
      texNJpsi5TeV->Draw();
    }
  ...
  TString texB = (isB==1)?"_B":"";
  TString texNjpsi = (isNjpsi>0)?(isNjpsi==1?"_Njpsi2TeV":"_Njpsi5TeV"):"";
  ...
  canvasRAA->SaveAs(Form("plotRAA/canvasRAA%s%s%s%s%s_cent_%.0f_%.0f.pdf",texAlice.Data(),texHadDupl.Data(),texB.Data(),texNjpsi.Data(),texTheory.Data(),centmin,centmax));
  // canvasRAA->SaveAs(Form("plotRAA/canvasRAA%s%s%s%s%s_cent_%.0f_%.0f.png",texAlice.Data(),texHadDupl.Data(),texB.Data(),texNjpsi.Data(),texTheory.Data(),centmin,centmax));
  if(isHadDupl==0&&isTheoryComparison==0&&isAlice==0&&isB==0&&isNjpsi==0) 
    canvasRAA->SaveAs(Form("plotRAA/canvasRAA_cent_%.0f_%.0f.C",centmin,centmax));

  if(verbose && isHadDupl==0&&isTheoryComparison==0&&isAlice==0&&isB==0&&isNjpsi==0)
    {
      for(int i=0;i<(hNuclearModificationMB->GetSize()-2);i++) 
        {
          ...
          std::cout<<std::setiosflags(std::ios::left)<<std::setw(15)<<hNuclearModificationMB->GetBinContent(i+1);
          std::cout<<std::setiosflags(std::ios::left)<<std::setw(15)<<hNuclearModificationMB->GetBinError(i+1);
          // std::cout<<std::setiosflags(std::ios::left)<<std::setw(15)<<gNuclearModificationMB->GetErrorYhigh(i);
          std::cout<<std::setiosflags(std::ios::left)<<std::setw(15)<<gNuclearModificationMB->GetErrorYlow(i)<<std::endl;
        }
      for(int i=0;i<(hNuclearModification->GetSize()-2);i++)
        {
          ...
          std::cout<<std::setiosflags(std::ios::left)<<std::setw(15)<<hNuclearModification->GetBinContent(i+1);
          std::cout<<std::setiosflags(std::ios::left)<<std::setw(15)<<hNuclearModification->GetBinError(i+1);
          // std::cout<<std::setiosflags(std::ios::left)<<std::setw(15)<<gNuclearModification->GetErrorYhigh(i);
          std::cout<<std::setiosflags(std::ios::left)<<std::setw(15)<<gNuclearModification->GetErrorYlow(i)<<std::endl;
        }
    }
}
...
int main(int argc, char *argv[])
{
  if(argc==10)
    {
      CombineRAA(argv[1], argv[2], atof(argv[3]), atof(argv[4]), atoi(argv[5]), atoi(argv[6]), atoi(argv[7]), atoi(argv[8]), atoi(argv[9]));
      return 0;
    }
  else
    { 
