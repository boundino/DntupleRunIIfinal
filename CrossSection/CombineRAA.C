...
void CombineRAA(TString fileMB="ROOTfilesCent10/outputRAAMB.root", TString file="ROOTfilesCent10/outputRAA.root", Float_t centMin=0., Float_t centMax=10., Int_t isHadDupl=1, Int_t isBnNjpsi=1, Int_t isTheoryComparison=2, Int_t isAlice=0, Bool_t verbose=false)
{
  ...
  TH2F* hemptyEff = new TH2F("hemptyEff","",50,xaxismin,xaxismax,10,0,1.75);
  ...
  */
  ...
  if((isHadDupl==1 && isBnNjpsi==0) || (isAlice==1 && isHadDupl==0 && isBnNjpsi==0)) legendSigma = new TLegend(0.594,0.76,0.594+0.46,0.89,"");
  ...
  if(isBnNjpsi>0 || isHadDupl || isAlice==1) ent_Dhighpt->SetTextSize(0.035);
  ...
  if(isAlice==1)
    {
      TLegendEntry* ent_Alice = legendSigma->AddEntry(grRAAalicesyst,"ALICE |y| < 0.5","pf");
      ent_Alice->SetTextFont(42); 
      ent_Alice->SetTextSize(0.035);
    } 
  ...
  if(isHadDupl==0&&isTheoryComparison==0&&isAlice==0&&isBnNjpsi==0) 
    {
